 ...
 * requête HTTP réelle contre le produit, dans son vrai moteur — `exports`
 * de `cloudflare:workers`, exposé par `@cloudflare/vitest-plugin`, appelle
 * le worker de production tel qu'il tournerait déployé (ADR-0003). Aucune
 * session ne peut exister à ce ticket : le garde de
 * `src/platform/session/index.ts` refuse tout, sans exception, et ce n'est
 * pas provisoire.
 *
 * `exports.default.fetch` exige que le worker principal soit résolu par
 ...
 *
 * Le module `cloudflare:workers` est typé par la déclaration ambiante du
 * projet (`src/platform/d1/cloudflare-workers.d.ts`), pas par
 * `@cloudflare/workers-types` : aucune référence de types n'est nécessaire.
 */
...
it('admin racine sans cookie renvoie vers l’écran de connexion', async () => {
  const reponse = await exports.default.fetch(new Request('https://example.com/admin/', { redirect: 'manual' }));
...
it('admin racine sans cookie ne rend jamais le contenu de l’accueil, on atterrit sur la connexion', async () => {
  const reponse = await exports.default.fetch(new Request('https://example.com/admin/'));
...
it('un chemin inconnu sous admin rend un refus', async () => {
  const reponse = await exports.default.fetch(new Request('https://example.com/admin/ceci-nexiste-pas'));
