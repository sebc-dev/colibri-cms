...
- **Où en est-on** — tableau d'avancement en tête du plan. **L1 à L6 sont faits** (PRD `FR-100` → `FR-113` ; [ADR-0011](docs/adr/ADR-0011-frontieres-de-contenu-hostile.md) « Frontières de contenu hostile », la racine que les lots suivants citent ; [ADR-0004](docs/adr/ADR-0004-architecture-du-code.md) amendement (c), ses suites dans le cœur ; [ADR-0010](docs/adr/ADR-0010-modele-brouillon-publie.md) amendement (c), clés naturelles, assets, cache et jeton de verrou ; [ADR-0007](docs/adr/ADR-0007-constructeur-de-formulaires.md) amendement (e), le chemin de soumission de bout en bout ; [ADR-0003](docs/adr/ADR-0003-socle-technique.md) amendement (d), la plateforme — exposition de la route publique, jeton D1, Deploy Hook, quotas, secrets, épinglage, facteur unique, tiers côté visiteur) ; le lot suivant est **L7**, l'amendement d'ADR-0006 **plus la promotion d'ADR-0009** (candidat dont les six contraintes sont déjà implémentées par le portail livré) — 4 constats : contrôles d'**innocuité** distincts des contrôles de conformité, gouvernance des dépendances, protection des hooks et de la CI, `SECURITY.md`.

Trois faits qui ne se dérivent d'aucun fichier : le **stash unique du dépôt est écarté** (il précède la réécriture documentaire du 2026-08-01 et son candidat « ADR-0010 » entre en collision avec l'ADR-0010 accepté) — c'est ce qui a permis au lot L2 de **prendre `ADR-0011`** ; `0009` reste réservé par `docs/adr/_candidates/`, si bien que le prochain numéro libre est `0012` ; et l'arbitrage porteur du lot L6 est que la route publique passe par un **motif de route sur le domaine du site**, jamais par un *Bypass* Access — parce qu'un motif de route vit dans le dépôt, où le portail peut le lire, alors qu'un Bypass vit dans le tableau de bord et n'y journalise rien. Côté PRD, `FR-113` est le maximum ; le prochain libre est `FR-114`.
