...
#
# Partagé par `suppression-guard` et `test-integrity`. La logique est critique
# et deux copies dériveraient ; ce fichier est donc surveillé par
...
# rien n'est accepté.
if [ ! -f "$signers" ]; then
  echo "::error title=Aucune clé de confiance::${signers} est absent — aucune preuve n'est possible, donc rien n'est accepté (docs/ci.md § Le garde de suppression)."
  exit 1
fi
