<!--
  TooltipContent — copié par la CLI shadcn-svelte (ticket 02,
  specs/002-socle-ilots-admin/02-composant-shadcn-sous-csp.md, ADR-0009,
  ADR-0010). Source amont : registre shadcn-svelte, style « vega », item
  `tooltip`. C'est ce composant que `ActionRapide.svelte` monte pour prouver
  ADR-0010 : `TooltipPrimitive.Content` (bits-ui, via Floating UI) pose un
  attribut `style="position: …"` en ligne, calculé à l'exécution, sur
  l'élément positionné — exactement ce que `style-src-attr 'unsafe-inline'`
  réconcilie avec la CSP stricte de l'administration.
-->
<script lang="ts">
