...
- [x] T21 — Écrire le test : une erreur de vérification de types en mode strict → `échoué` _Requirements: FR-010_ ; dépend de : —
- [x] T22 — Implémenter `typecheck` (`tsc --noEmit`) jusqu'à T21 vert _Requirements: FR-010_ ; bloqué par : T21
- [x] T23 [P] — Écrire le test : lint ou contrôle de format en violation → `échoué` _Requirements: FR-011_ ; dépend de : —
- [x] T24 — Implémenter `lint-format` (ESLint + `prettier --check`) + `prettier.config.mjs`/`.prettierignore` jusqu'à T23 vert _Requirements: FR-011_ ; bloqué par : T23
- [x] T25 [P] — Écrire le test : une migration se terminant par un commentaire → `échoué` (bug #7739) _Requirements: FR-012_ ; dépend de : —
- [x] T26 — Implémenter `migration-comment` jusqu'à T25 vert _Requirements: FR-012_ ; bloqué par : T25
- [x] T27 [P] — Écrire le test : une version structurante hors catalogue centralisé, ou des majeures site/adaptateur mélangées → `échoué` _Requirements: FR-013_ ; dépend de : —
- [x] T28 — Implémenter `versions-catalog` (`package.json` vs `catalog:`) jusqu'à T27 vert _Requirements: FR-013_ ; bloqué par : T27
...
- [x] T29 — Écrire le test : la suite d'intégration (ressources de données locales) échoue → contrôle `échoué` _Requirements: FR-001_ ; dépend de : —
- [x] T30 — Implémenter `integration` (vitest-pool-workers) jusqu'à T29 vert _Requirements: FR-001_ ; bloqué par : T29
- [x] T31 [P] — Écrire le test : un endpoint d'écriture sans test d'autorisation → `échoué` ; 100 % couverts → `passé` _Requirements: FR-007_ ; _SC-005_ ; dépend de : —
- [x] T32 — Implémenter `authz-coverage` (mapping endpoints↔tests) jusqu'à T31 vert _Requirements: FR-007_ ; bloqué par : T31
- [x] T33 [P] — Écrire le test : une route publique sans test de rejet en l'absence de jeton anti-robot valide → `échoué` _Requirements: FR-008_ ; dépend de : —
- [x] T34 — Implémenter `turnstile-test` jusqu'à T33 vert _Requirements: FR-008_ ; bloqué par : T33
- [x] T35 — Écrire le test : aucune route publique existante → contrôle Turnstile `ignoré` (jamais `échoué`) _Requirements: FR-009_ ; bloqué par : T34
- [x] T36 — Implémenter la sémantique `ignoré` à périmètre vide (`applies` faux) jusqu'à T35 vert _Requirements: FR-009_ ; bloqué par : T35
