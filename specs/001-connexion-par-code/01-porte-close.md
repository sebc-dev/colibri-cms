...
- [ ] `/admin/` demandé sans cookie de session rend un renvoi vers l'écran de connexion, et jamais le contenu de l'accueil
- [ ] l'écran de connexion se rend avec comme sans cookie de session, et porte son champ d'adresse
- [ ] un chemin inconnu sous `/admin/` rend un refus, sans rien laisser voir de l'administration
- [ ] l'écran d'accueil existe et ne porte aucune fonction — ni lien vers un autre écran, ni action ; aucune requête ne l'atteint tant qu'aucune session ne peut s'ouvrir
- [ ] `src/pages/admin/index.astro` et `src/pages/admin/connexion.astro` importent tous deux le garde `src/platform/session/index.ts` (ADR-0007)
- [ ] aucun gabarit sous `src/admin/` ne porte de directive `client:*` ni de bloc `<style>` — la feuille de style est un asset lié (ADR-0006, ADR-0008)
- [ ] aucun terme de développeur ne paraît dans les textes rendus
