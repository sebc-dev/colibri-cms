...
> jamais sur une préférence d'agent. Les faits proviennent des rapports de `docs/research/`
> et de vérifications faites sur la machine, citées ici avec leur emplacement. Un fait non
> sourcé est marqué comme tel : il ne descend pas dans un ADR. **Toutes les dates de ce
> document sont en `AAAA-MM-JJ`** — celles du projet comme celles des sources citées.
...
|---|---|---|---|
| Langage | TypeScript, mode strict | (tous) | [ADR-0010](./adr/0010-langage-typescript-strict.md) |
| Générateur du site public et de l'aperçu | Astro 7 | FR-081, FR-095, FR-096, SC-005 | [ADR-0002](./adr/0002-generateur-astro-7.md) |
| Cible de déploiement et système de build | Un Worker Cloudflare unique — assets statiques + routes serveur —, bâti par Workers Builds | FR-087, FR-089, FR-096, FR-097, SC-001, SC-004 | [ADR-0001](./adr/0001-cible-de-deploiement-worker-unique-workers-builds.md) |
| Base de données | Cloudflare D1 — brouillons, état publié, demandes. **Ce que D1 porte seul — brouillons, médias en brouillon, suite donnée — n'a aucune copie, et sa reprise après perte n'est pas établie : réserve au candidat n° 3** | FR-026, FR-032, FR-044, FR-051, FR-065, FR-078, FR-092 | [ADR-0003](./adr/0003-magasin-d1-brouillons-etat-publie-et-demandes.md) |
| Contenu publié en fichiers | Un répertoire par objet dans le dépôt : `page.json` pour la structure, un `.md` par emplacement de texte riche | FR-087, FR-107, FR-109, SC-011 | [ADR-0007](./adr/0007-format-du-contenu-un-repertoire-par-objet.md) |
| Médias publiés | Même dépôt, branche orpheline `media` : dépôt **additif** à la publication, **élagage des orphelins au début de la publication suivante** | FR-037, FR-084, FR-088, FR-107, FR-108, SC-011 | [ADR-0004](./adr/0004-medias-deux-magasins-un-par-etat.md) |
| Médias en brouillon | D1 : le binaire est stocké en `BLOB`, transféré sur `media` à la publication puis effacé de la base. Plafond **2 Mo par média**, imposé par D1 et non choisi | FR-027, FR-033, FR-034, FR-040, SC-010 | [ADR-0004](./adr/0004-medias-deux-magasins-un-par-etat.md) |
| Ingestion des médias | Liste blanche fermée — **JPEG, PNG, WebP**, **SVG refusé** — reconnue sur les **octets d'en-tête** du fichier, jamais sur l'extension ni sur le `Content-Type` du téléversement ; le `Content-Type` servi est déduit de cette liste | FR-027, FR-040, FR-108 | [ADR-0014](./adr/0014-ingestion-des-medias-liste-blanche-sur-octets.md) |
| Forge et écriture de la publication | GitHub ; API REST *git data* — **contenu textuel inliné** dans les entrées de `POST /git/trees`, **médias déposés par `POST /git/blobs` en base64** — puis `PATCH /git/refs` en `force: false`, avance rapide obligatoire — **sauf l'élagage de `media`**, seul geste non-avance-rapide, exécuté sous le verrou et calculé depuis D1. Jeton à portée fine, sans expiration, permission `Contents: Read and write` **seule** | FR-086, FR-089, FR-091 | [ADR-0005](./adr/0005-forge-github-api-git-data-jeton-a-portee-fine.md) |
| Déclenchement du build | Workers Builds surveille la branche `main` **seule** ; le build récupère `media` pendant son exécution | FR-089, FR-107, SC-011 | [ADR-0001](./adr/0001-cible-de-deploiement-worker-unique-workers-builds.md) |
| Maintien en vie du jeton d'écriture | Cron Trigger dans le compte de la cliente, appel anodin **hebdomadaire** — la cadence *est* la parade : le Cron du palier gratuit n'a **pas de retry**, un appel sauté n'est jamais réémis, et 52 passages par an laissent la marge que la fenêtre glissante d'un an de GitHub absorbe | FR-101, SC-012 | [ADR-0005](./adr/0005-forge-github-api-git-data-jeton-a-portee-fine.md) |
| Auth | Implémentation maison sur D1, mécanisme par mécanisme : **code à saisir** — 40 bits, haché, usage unique, expirant, **lié au navigateur demandeur**, brûlé au 5ᵉ essai — et **jamais un lien** ; **session opaque en D1**, donc **sans clé de signature**, expirant à **sept jours d'inactivité et trente jours d'âge** ; cookie `__Host-`, `HttpOnly`, `Secure`, `SameSite=Strict`, `Path=/` ; **jeton anti-CSRF sur chaque écriture**, doublé d'un contrôle d'en-tête `Origin` | FR-001 à FR-008, FR-082, FR-118, FR-120 à FR-122, SC-006, SC-021 | [ADR-0006](./adr/0006-auth-implementation-maison-sur-d1.md) |
| Moyen de reprise | Code de **128 bits** — 26 caractères base32, groupés pour la recopie — **haché en D1**, remis sur papier à la livraison, **à usage unique et réémis à l'emploi** ; **aucun frein par secret**, l'entropie seule rend la devinette sans objet, en ligne comme sur fuite de la base — rien en configuration du déploiement (`FR-011`), aucune dépendance à un tiers | FR-009 à FR-012, SC-020 | [ADR-0016](./adr/0016-moyen-de-reprise-code-128-bits-hache.md) |
| Acheminement des demandes | Cloudflare Email Routing, binding `send_email` vers l'adresse de destination **vérifiée** ; e-mail **inerte et étiqueté** — texte seul, objet fixe posé par le produit, chaque donnée du visiteur rendue derrière son étiquette, aucun lien ni mise en forme construits depuis sa saisie — **produit en bêta publique, réserve au candidat n° 9** | FR-063, FR-064, SC-007 | [ADR-0009](./adr/0009-acheminement-email-routing-send-email.md) |
| Moyen anti-abus | Turnstile en mode *managed* devant, puis compteur de fréquence dans un Durable Object **unique**, qui porte une **table d'origines** — jamais un objet par visiteur. Chaque origine y entre sous une empreinte **HMAC, sous une clé tirée au hasard par le produit pour la seule fenêtre de comptage en cours** ; clé et entrées **effacées ensemble** à la fin de la fenêtre | FR-007, FR-062 | [ADR-0012](./adr/0012-anti-abus-turnstile-et-compteur-a-empreintes-de-fenetre.md) |
| Sérialisation et suivi des publications | Une **seule** ligne d'état en D1 : verrou conditionnel, **bail horodaté** repris à l'expiration, et **issue du dépôt** | FR-090, FR-091 | [ADR-0017](./adr/0017-serialisation-des-publications-et-constat-de-mise-en-ligne.md) |
| Constat de la mise en ligne | Le site publié expose l'empreinte du commit dont il est né ; l'administration la lit par une requête **publique** et la compare | FR-090 | [ADR-0017](./adr/0017-serialisation-des-publications-et-constat-de-mise-en-ligne.md) |
| Framework d'îlots — administration et pages publiques | Îlots Svelte 5 dans Astro | FR-017, FR-054, FR-117, SC-003, SC-005, SC-015 | [ADR-0011](./adr/0011-ilots-svelte-5.md) |
| Texte riche | Éditeur TipTap, sérialisation en **Markdown restreint** aux marques testées ; au rendu, **liste blanche de schémas d'URL** (`https`, `mailto`, `tel`, relatif) et **aucun HTML brut** | FR-018, FR-117, SC-011 | [ADR-0008](./adr/0008-texte-riche-markdown-restreint.md) |
| En-têtes de réponse | **Deux porteurs, imposés par la plateforme** : un fichier `_headers` pour les pages publiques, servies en assets statiques ; les mêmes en-têtes posés **dans le code** pour l'administration, l'aperçu et les médias servis depuis D1, **dont une CSP stricte propre à l'administration** — seule parade qui subsiste au XSS same-origin tant que l'origine reste commune | FR-082, FR-095, FR-096 | [ADR-0015](./adr/0015-en-tetes-de-reponse-deux-porteurs.md) |
| Pipeline d'images | `image.layout: 'constrained'`, `image.breakpoints: [640, 960, 1280]`, `<Image>` à un seul format | SC-005, SC-001 (par `C5`) | [ADR-0019](./adr/0019-pipeline-d-images-variantes-au-build.md) |
| Accès aux données | API D1 native, migrations `wrangler d1 migrations` | FR-105, FR-106, SC-008 | [ADR-0018](./adr/0018-acces-aux-donnees-api-d1-native-et-migrations-wrangler.md) |
| Configuration d'instance | **Quatre lieux, un par nature de valeur.** Les **liaisons de plateforme** — rattachement D1, `send_email`, Durable Object, Cron — dans la configuration du déploiement, seul endroit possible : un rattachement ne peut pas vivre dans la base qu'il ouvre. Les **secrets** dans le compte Cloudflare de la cliente, comme la clé **de vérification** Turnstile. Ce qui doit **changer sans redéploiement** — l'adresse autorisée — en D1. Tout le reste, **domaine et clé publique Turnstile compris**, dans un **fichier d'instance unique et versionné** qu'aucune montée de version ne touche : `C6` exige qu'un clone nu, sans D1 ni accès Cloudflare, trouve dans les fichiers de quoi bâtir le site — le domaine pour ses URL canoniques, la clé publique pour le widget que porte toute page à formulaire. **Aucun secret n'y entre** — le dépôt est ouvert à l'intégrateur comme collaborateur, et la clé publique n'en est pas un : c'est le widget lui-même qui la publie au visiteur | FR-104, FR-105, SC-008 | [ADR-0020](./adr/0020-configuration-d-instance-quatre-lieux.md) |
| Tests | Vitest dans `workerd` via `@cloudflare/vitest-pool-workers`, Playwright pour les parcours, épreuve de réversibilité scriptée | (tous) ; SC-003, SC-009, SC-011, SC-016 | [ADR-0013](./adr/0013-tests-vitest-dans-workerd.md) |
| Détection de panne d'acheminement | État d'acheminement porté par chaque demande et affiché dans la liste | **— exigence à créer**, voir ci-dessous | — (aucun candidat ADR : choix sans exigence porteuse) |
...
2026-08-11 y ferait disparaître en silence un média fraîchement déposé, là où un `force: false`
...
(docs Cloudflare D1 · *Limits*, page datée du **2026-04-21**) :
...
Docs Cloudflare · *Workers · Static Assets · Headers*, page datée du **2026-04-23** :
...
*Pricing*, datée du 2026-07-07), donc hors du quota de 100 000 requêtes par jour. Mais
...
Mesure du 2026-08-11, sur dépôt jetable et avec témoin :
...
access token* ; fichier source `github/docs` au commit `6f9f6f89` du **2026-06-23**). Comme
...
Ce point était ouvert. Il a été fermé le **2026-08-11** par une série de mesures sur un dépôt
...
en tête : le 2026-08-12, le traitement de `S-10` a montré qu'un même fait pouvait être mesuré ici
et cité là.
...
du 2026-08-11 n'a jamais été versée ici ; **quatre** lignes portent donc `[mesuré · trace non versée]`,
...
écrit de son écartement au candidat ADR n° 5, et elle ne se citera jamais. **Arbitré le 2026-08-12
...
| `POST /git/blobs`, `/git/trees` et `/git/commits` n'exigent que **`Contents: write`** | [officiel · cité] `progAccess.permissions` vaut `{"Contents": "write"}` pour les trois — donnée source de la doc GitHub, `github/docs` › `src/rest/data/fpt-2026-03-10/git.json`, commit `0b2db291` du 2026-06-23, identique en version d'API `2022-11-28`. Corrobore la mesure du 2026-08-11 (aucun refus sur la chaîne) |
| **Le déplacement de ref, lui, a *deux* jeux de permissions suffisants** : `Contents: write`, **ou** `Contents: write` + `Workflows: write` | [officiel · cité] même source, `POST /git/refs` et `PATCH /git/refs/{ref}`. La mesure du 2026-08-11 ne pouvait pas le voir : le dépôt d'essai n'avait aucun fichier de workflow. *Lecture, non citée : le second jeu se lève quand le commit visé touche `.github/workflows/`* — voir la contrainte au candidat ADR n° 5 |
...
options. Les trois qui portaient sur le jeton d'écriture ont été retirés le 2026-08-11 —
...
1. **Cible de déploiement et système de build : un Worker unique bâti par Workers Builds.**
   ...
   a retiré le support de Pages à la **v13.0.0, publiée le 2026-03-10** : le plan de routage
   ...
   *Complété le 2026-08-12 par le traitement de `S-10` et de `S-11`, qui reprochaient à cette
   ...
   événement** : `@astrojs/cloudflare@13.0.0` et `astro@6.0.0` ont été publiés le 2026-03-10 à
   ...
   6 (`6.0.0-alpha.0`, 2025-11-10). Les deux dates coexistent, et un lecteur qui remonte au
   ...
   qu'à la 13.1.3, le 2026-03-20 —, et le mot `pages` figure bien trois fois dans son `dist`,
   ...
   *Étendue le 2026-08-12 par le traitement de `S-17`, qui a trouvé la ligne « Déclenchement du
   build » sans candidat : née le 2026-08-11 du traitement de `S-08`, elle est postérieure au constat,
...
4. **Médias : deux magasins, un par état — branche orpheline `media` pour le publié, additive
   ...
   started », MAJ 2026-04-21) qui **souscrit un service facturé à l'usage sur le moyen de
...
5. **Forge et écriture de la publication : GitHub, API REST *git data* puis
   ...
   *Complété le 2026-08-12 par le cinquième grief de `S-10`. L'argument « une seule permission »
   ...
   n'en ont qu'un. La mesure du 2026-08-11 ne pouvait pas le voir : le dépôt d'essai n'avait aucun
...
6. **Auth : implémentation maison sur D1.** Retenue car la surface est exactement le besoin —
   ...
   **22 paquets** et sa version bouge à la semaine (`1.6.26` le 2026-08-04, `1.6.27` le
   2026-08-11), sur le chemin d'accès unique à l'administration ; et le **moyen de reprise**
...
12. **Anti-abus : Turnstile *managed* devant, compteur de fréquence dans un objet unique, à
    ...
    *managed*** (docs Turnstile *Plans*, 2026-04-16, « Unlimited challenges »), il absorbe le
    ...
    **invisibles** est tenue d'une **analyse tierce**, marquée `[INCERTAIN]` par*
    ...
    même pas de période, aucun calcul ne la fixerait sans refaire l'erreur écartée le 2026-08-10
...
13. **Tests : Vitest dans `workerd`, Playwright, épreuve de réversibilité scriptée.** Retenue
    ...
    **`vitest ^4.1.0`**, donc une version majeure imposée, et une chaîne de moteur épinglée au
    ...
    et corrige les deux — `0.21.0` a été publiée le 2026-08-10 et dépassée deux fois en moins de
    48 h (`0.21.2` le 2026-08-12), si bien qu'un numéro de correctif n'a pas sa place dans un
...
15. **En-têtes de réponse : deux porteurs — `_headers` pour le public, le code pour tout ce que
    ...
    datée du **2026-04-23**). Comme `FR-095` et `FR-096` font des pages publiques des assets
    statiques, le fichier les couvre sans coût — « Requests to static assets are free and
    unlimited » (page *Pricing*, datée du 2026-07-07) —, tandis que l'administration, l'aperçu
