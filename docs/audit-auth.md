...
| AU-08 | Majeur | **Confirmé, et refermé au PRD et au socle §7 : l'état d'authentification est déclaré non durable par choix, et sa perte se répare par un réamorçage — le geste de livraison rejoué —, sans aucun mécanisme de sauvegarde ; Stack inchangé.** L'instruction a confirmé le constat sur pièces : adresse autorisée, empreinte du moyen de reprise et sessions vivent en D1 (`stack.md:37-38`), aucune n'est du contenu, donc `I2`/`I3` (`brief.md:207-211`) ne les couvrent pas, et `FR-011` ferme l'échappatoire (`prd.md:357`) — une perte de base rend l'instance inadministrable, sans attaquant, la même issue que le cas limite « ferme la porte derrière lui ». **Deux nuances ont décidé** : `FR-011` est justifié mais fut écrit contre un attaquant, jamais éprouvé contre une panne — effet de bord du magasin D1 ; et les sessions sont sans enjeu, elles expirent de toute façon (`FR-118`) — ne restent que l'adresse (l'e-mail du client) et l'empreinte, toutes deux **réensemençables**. Réétablir l'adresse et engendrer un **nouveau** moyen de reprise **est** le geste de livraison (`FR-009`), rejoué : aucun mécanisme neuf, seulement une procédure à écrire pour exister le jour venu. **Écartés** : exiger la durabilité avec un porteur technique (sauvegarde/réplication D1) — une seconde copie durable de l'empreinte, exactement la surface que `FR-011` réduit ; un gain limité à « garder le même papier » quand réémettre est un geste une fois par incident (profil de coût accepté en `AU-03`) ; et un choix de techno de sauvegarde qui sortirait de l'audit sur pièces pour une décision Stack à mesurer, renvoyée au premortem. Ne rien faire, `SC-014` couvrant — `SC-014` couvre redéployer+publier (le contenu), pas le réamorçage de l'auth, et le trou est précisément l'absence de procédure écrite. **Conforme à `FR-011`** : le réamorçage crée un nouveau moyen de reprise, il ne conserve ni ne reconstitue l'ancien ; **exécutable après la panne** depuis les seuls accès Cloudflare du client (la base lui appartient), conforme à `SC-014` et `I4`. Portés le 2026-08-12 : `FR-119` (PRD, famille « Dossier d'instance »), cas limite « La base de l'instance est perdue » (PRD), ligne de recette §7 (`socle-de-livraison.md`). Stack non modifié — aucun porteur technique n'est choisi, c'est le résultat même de l'arbitrage. |
| AU-09 | Majeur | **Confirmé — et un peu plus large que le constat : refermé au PRD seul, `FR-008` cessant de promettre l'indistinguabilité en général pour tenir exactement ce qui la vaut, la fermeture du balayage.** L'instruction a vérifié la mécanique : le plafond de `FR-006` n'est atteignable que pour l'adresse autorisée, puisque `FR-005` interdit tout autre envoi — sa manifestation la désigne. Elle a **élargi la prise** : le mot « réponse » de `FR-008` était étroit, il ne couvrait ni le délai ni un changement d'état, et rien au dépôt n'exigeait que le chemin qui envoie un e-mail réponde comme celui qui n'envoie rien — la ligne « Auth » de `stack.md:37` ne dit rien de l'envoi. Elle a aussi trouvé une **contradiction déjà présente** : le scénario 2 d'`US1` promettait « rien ne révèle quelle adresse est autorisée », absolu, quand `FR-006` exige un plafond qui révèle — deux textes du même document, sans arbitre. Le rejeu a ensuite pesé ce que la promesse protège : l'intention d'`A-01` visait **l'énumération par balayage**, soit une soumission par adresse candidate, et `AU-12` — requalifié en constat accepté — a déjà déclaré l'adresse divulguée par l'usage normal, le constat lui-même la disant vraisemblablement publiée sur le site de la cliente. **La promesse est donc bornée à ce qui la vaut** : même réponse et même délai pour toute adresse, l'un et l'autre indépendants de l'envoi effectif, ce qui ferme le balayage exactement ; le reste est **écrit comme non promis** plutôt que laissé en promesse intenable — ce que le constat réclamait, « une promesse intenable est pire qu'une promesse absente ». **Résidu assumé, et nommé au PRD** : marteler une même adresse jusqu'au plafond la désigne encore ; l'attaquant paie N e-mails vers la vraie boîte pour apprendre ce qui est écrit sur le site de la pâtissière, et le plafond reste installable à volonté — `AU-03` a établi qu'un attaquant distribué contourne `FR-007` par le nombre d'origines. **Écartés** : étendre `FR-008` à tout comportement observable, plafond compris — l'écran aurait dit « c'est parti » à l'éditrice bloquée, un succès affirmé faux, là où le cas limite « l'acheminement cesse de fonctionner en silence » n'accepte qu'une **panne** que personne n'a choisie ; le PRD sait faire l'inverse (`FR-060` informe le visiteur quand un envoi n'aboutit pas), et le prix aurait été payé pour un secret déjà perdu ; le **plafond universel** — appliquer le même plafond à toute adresse saisie ferme la fuite et garde la franchise, mais exige un compteur par adresse, non borné et créable à volonté, une surface d'abus neuve sur un produit dont l'Annexe A compte les requêtes, et le PRD y prescrirait un mécanisme au lieu d'un effet ; **supprimer `FR-008`** — le scénario 2 d'`US1` redeviendrait l'orphelin qu'`A-01` avait trouvé. **Renvoyé à `AU-10`** : le critère de succès qui mesurera cette exigence — aucun `SC` ne couvre `FR-005` à `FR-008`, et c'est son constat, pas celui-ci. Portés dans `prd.md` : `FR-008` réécrite, scénario 2 d'`US1` réaligné, exclusion « Secret de l'adresse autorisée » (**nouvelle**, § « Arrêté par ce PRD »). |
| AU-10 | Majeur | **Confirmé dans ses deux moitiés, et refermé par un porteur par registre : une exigence là où le mécanisme est un comportement, un contrôle bloquant là où il est une propriété statique, plus l'épreuve qui manquait côté abus.** L'instruction a relu `FR-001` à `FR-008` et `FR-118` — aucune ne nomme la liaison au navigateur demandeur, le brûlage, les attributs du cookie ni le jeton anti-CSRF — puis les vingt critères de succès : `SC-006` compte les comptes, `SC-020` éprouve la reprise, **aucun ne mesure quoi que ce soit de `FR-005` à `FR-008`**. Les deux moitiés tiennent. **La prise s'est élargie** : la ligne « Auth » dit aussi le code **à usage unique et expirant**, et cela non plus n'était porté par rien — le constat ne l'avait pas compté. Ce n'est pas un détail sur ce produit : une preuve qui ne périme pas et se rejoue est un sésame permanent posé dans une boîte e-mail, c'est-à-dire le scénario autour duquel toute la conception tourne (`AU-04`, « le lecteur de la boîte, le scénario le plus probable »). **Un résidu de `AU-09` a été trouvé en chemin et corrigé** : le candidat ADR n° 6 affirmait encore que `FR-005` et `FR-008` « y sont tenus par construction », en citant l'ancienne rédaction de `FR-008` — du couvert par affirmation, dans le document même que ce constat devait amender ; `FR-005` l'est bien, mais par la plateforme (`send_email` n'écrit qu'à une destination vérifiée), et `FR-008` ne l'est pas. **La répartition retenue** suit la nature de chaque mécanisme : les **comportements** descendent en exigences — liaison à l'appareil demandeur (`FR-120`), usage unique et durée bornée (`FR-121`), brûlage après un nombre borné de présentations erronées (`FR-122`) —, les **propriétés statiques** rejoignent la vérification mécanique en 8ᵉ contrôle bloquant — attributs du cookie, jeton anti-CSRF doublé du contrôle d'`Origin` —, sur le modèle exact retenu pour la CSP au traitement de `AU-06`, que l'impact du constat désignait lui-même comme le même trou en plus étroit ; et l'ensemble se mesure par `SC-021`, épreuve de résistance de la connexion, pendant côté abus de `SC-014` et `SC-020`. **Dérive corrigée au passage** : le § « Ce que cette phase dépose » comptait **six** contrôles quand la section en listait sept depuis `AU-06` — porté à huit et daté constat par constat ; sa ligne « `prd.md` non modifié » est requalifiée, la phase Stack ne le modifiant pas, les traitements de cet audit si. **Écartés** : les deux gestes de la piste seuls (`SC` nouveau + une phrase au candidat n° 6 exigeant la descente en `SHALL`) — la phrase est un vœu que rien n'exécute, la Stack n'étant pas un document que la `ci` vérifie, ce que l'impact du constat écrit noir sur blanc, et une épreuve `SC` est un critère **produit** mesuré une fois, qui n'empêche aucun lot d'omettre le brûlage : on aurait refermé le trou du vœu par un vœu, le défaut que `AU-05` avait nommé — « le trou d'`AU-10` reproduit au moment de le refermer » ; **tout en contrôle bloquant** — deux des mécanismes sont des comportements, non des propriétés lisibles dans les sources ; leur contrôle serait un test, et un test se dérive de la `SHALL` qui manque précisément, le raisonnement tourne en rond ; **descendre les quatre mécanismes en `FR`** — le PRD aurait prescrit des mécanismes (« lié au navigateur demandeur », « jeton anti-CSRF ») au lieu d'effets, quittant son registre et doublant la Stack. **Résidu versé au dossier de `/scd-sdd:premortem socle`** : la ligne « Moyen de reprise » a le même défaut — « à usage unique et réémis à l'emploi » n'est porté par aucune exigence — mais c'est une autre ligne de la Stack, hors du périmètre de ce constat. Portés dans `prd.md` : `FR-120`, `FR-121`, `FR-122` et `SC-021` (**nouveaux**). Portés dans `stack.md` : backref de la ligne « Auth » étendue, § « Vérification mécanique obligatoire » porté de sept à huit contrôles, candidat ADR n° 6 amendé (phrase « par construction » corrigée, porteur nommé mécanisme par mécanisme), § « Ce que cette phase dépose » rectifié. |
| AU-11 | Mineur | **Confirmé, et refermé au PRD seul : l'espace qui porte le dossier n'est pas ouvert plus largement que ce que le dossier décrit — la clause rejoint `FR-110`, l'exigence qui plaçait déjà le dossier sans le borner.** L'instruction a confirmé le constat sur pièces — `FR-111`, `FR-112`, `FR-113` agrègent bien la carte, `FR-110` ne disait que l'appartenance de l'espace — puis a resserré **où l'agrégation mord**. Le dossier ne donne rien à qui n'a pas les accès : `SC-014` le mesure ainsi, un tiers muni « des seuls accès aux comptes de la cliente **et** du dossier ». Il reste un point, un seul, où le plan devient une clé — l'emplacement d'un **objet physique** : le moyen de reprise est un code sur papier rangé chez la cliente, son emplacement noté au dossier (`stack.md`, ligne « Moyen de reprise »). Les **128 bits** chiffrés en `AU-03` portent la devinette, jamais la carte : contre qui lit l'emplacement et prend le papier, l'entropie ne peut rien. C'est cette voie que la clause ferme, et elle explique pourquoi un Mineur méritait une exigence plutôt qu'une phrase. **Le rejeu adverse principal ne tient pas** : « le produit ne contrôle pas l'espace de la cliente, l'exigence serait décorative » — `FR-110` **prescrit déjà** cet espace, et le dépôt est un geste de livraison, vérifiable au même moment et de la même façon que l'emplacement lui-même ; la borne est **comparative**, donc observable — qui ouvre l'espace, contre les comptes de `FR-111` et les rangements de `FR-112`. La cohérence est portée jusqu'au scénario : `US14` scénario 3 était la forme observable de `FR-110`, il gagne la même borne, faute de quoi le PRD aurait gardé un scénario sous-couvrant son exigence. **Écartés** : la **phrase au Brief seul** — c'est la piste écrite au constat, mais elle reproduirait la forme d'`AU-10`, refermé la veille sur le défaut exact « un mécanisme sans exigence porteuse » ; et aucun des dix arbitrages rendus n'a touché `brief.md`, le Brief restant la racine dont tout descend ; un **volet à `SC-013`** — cette épreuve mesure la non-rémanence des accès de l'intégrateur, y greffer l'ouverture d'un espace tiers mêlerait deux sujets, et chargerait d'un critère de succès un constat que l'audit qualifie lui-même d'hygiène ; le **constat accepté** à « NON inclus » — il aurait fallu poser que le produit ne peut rien dire de l'espace, quand l'exigence d'à côté le prescrit ; **retirer du dossier l'emplacement du moyen de reprise** — la seule mesure qui supprimerait l'agrégation à sa source, mais le cas limite « L'éditrice perd l'accès à sa boîte » s'appuie explicitement sur `FR-112` pour cela, et `FR-119` charge le dossier de la procédure de réétablissement : le dossier deviendrait muet là où il sert. **Limite assumée** : la clause tient **au moment du dépôt** et personne ne revérifie un partage six mois plus tard — un espace ouvert après coup par la cliente échappe à toute exigence du produit ; ce résidu, la dérive de l'espace après livraison, part au dossier de `/scd-sdd:premortem socle`. Portés dans `prd.md` : `FR-110` amendée, `US14` scénario 3 aligné. |
