...
- **FR-061** : À l'envoi d'une soumission, le système DOIT acheminer à l'adresse e-mail du formulaire les réponses saisies, le total éventuel et les coordonnées du visiteur. Lorsque le formulaire recueille l'adresse e-mail du visiteur, une **réponse** au message acheminé DOIT parvenir directement à ce visiteur.
- **FR-095** : Le système DOIT adresser au visiteur une copie de sa soumission, lorsque le formulaire recueille son adresse e-mail.
...
- **FR-064** : Le système NE DOIT PAS conserver une soumission au-delà de son acheminement **réussi**. En cas d'échec, il DOIT la retenir le temps de réessayer, et l'effacer dès la livraison ou à l'expiration d'un délai borné. *(Cette rétention est strictement au service de l'acheminement : aucun écran, aucune liste, aucune recherche — la « base de prospects » reste hors périmètre.)*
- **FR-094** : Un échec définitif d'acheminement DOIT être signalé à l'éditrice dans l'espace d'édition (FR-087).
...
- **Acheminement d'une soumission en échec.** L'échec d'un envoi d'e-mail se manifeste souvent **après** que le visiteur a reçu sa confirmation. Le système retient alors la soumission le temps de réessayer (FR-064), signale un échec définitif à l'éditrice dans l'espace d'édition (FR-094), et le visiteur dispose de sa propre copie (FR-095) pour relancer. Aucune demande n'est perdue en silence.
...
- **Conservation et suivi des soumissions** : pas d'écran de suivi, pas de base de prospects, pas de statut « traité ». La rétention transitoire de FR-064 est au seul service de l'acheminement et n'expose aucune surface.
...
- **SC-007** — La cliente construit un formulaire de devis elle-même, et une demande composée par un visiteur lui parvient par e-mail, avec réponses, total et coordonnées. *Servi par* : FR-040 à FR-046, FR-061, FR-086, FR-091, FR-094, FR-095, FR-096.
...
- **Durée de session et révocation d'accès.** FR-001 refuse l'accès aux personnes non autorisées, mais rien ne dit combien de temps une session d'éditrice reste ouverte, ni par quel geste l'intégrateur coupe l'accès le jour où une cliente quitte l'agence. Relève du seam d'authentification ([stack.md](./stack.md) / ADR), pas du PRD — mais la **décision d'exploitation** reste à prendre.
...
Rappel de séquencement : le constructeur de formulaires (US6, US7, FR-040 à FR-052, FR-060 à FR-065, FR-086, FR-090, FR-091, FR-094 à FR-096) est **P2**. Il dépend du socle P1 (édition, médias, publication) et ne doit pas être entamé avant que celui-ci soit en état.
