...
- **FR-111** : Le retrait d'une page du site public (FR-083) DOIT être reflété par le site servi au visiteur dans le même délai qu'une publication (FR-036). *(Ajoutée le 2026-08-01 à la suite de l'[audit de sécurité](./audit-securite-2026-08-01.md). FR-035 décrit l'état du **build**, pas ce que le visiteur reçoit : sans cette exigence, une page retirée pouvait rester servie jusqu'à expiration d'un cache que rien ne bornait. **Frontière de ce qui est garanti** : la page cesse d'être atteignable depuis le site ; une image dont quelqu'un détient déjà l'adresse directe peut, elle, rester résolvable — le produit n'offre aucun geste de suppression de média en v1, voir « Fichiers non référencés » en Cas limites.)*
...
- **FR-112** : Le système DOIT refuser toute soumission visant un formulaire qui n'est plus exposé sur le site public (FR-083). *(Ajoutée le 2026-08-01 à la suite de l'[audit de sécurité](./audit-securite-2026-08-01.md). Le retrait d'un formulaire ne supprime pas sa définition publiée — c'est ce qui permet de le remettre en ligne inchangé —, si bien que « valider contre la définition publiée » (FR-090) restait vrai après le retrait : l'adresse d'envoi et les noms de champs étant connaissables depuis un ancien cache, une surface que l'éditrice croit fermée continuait d'accepter des demandes et de collecter des données personnelles. Retirer un formulaire du site ferme donc aussi la collecte.)*
...
- **FR-113** : L'information de confidentialité (FR-105) DOIT nommer les services tiers susceptibles d'être chargés dans le navigateur du visiteur à la suite d'une action de sa part — le dispositif anti-robot et le lecteur vidéo —, indiquer ce que chacun dépose sur son terminal, et préciser qu'une page seulement consultée n'en charge aucun. *(Ajoutée le 2026-08-01 à la suite de l'[audit de sécurité](./audit-securite-2026-08-01.md). FR-089 garantit qu'aucun tiers ne s'exécute avant une action du visiteur — mais c'est une règle de **conception**, pas un mécanisme de consentement : un clic sur « lire » ne vaut choix éclairé que s'il est **précédé** de l'information. FR-105 énumérait ce que la mention doit dire et ne parlait pas de ces tiers.)*
...
- **Fichiers non référencés.** Un fichier téléversé puis abandonné (page quittée sans enregistrer, image remplacée) **reste stocké** : le produit n'offre aucun geste de suppression de média (médiathèque hors périmètre), et n'en récupère aucun automatiquement en v1. C'est une décision, pas un oubli — la réduction des images à l'entrée (FR-088) maintient le volume très en deçà du quota gratuit, et le nettoyage éventuel relève de l'intégrateur. *Toute récupération automatique future devra tenir compte des **deux** contenus (FR-078) : une image absente du contenu en cours peut être servie par le contenu en ligne.* **Le retrait d'une page relève du même renoncement** *(2026-08-01)* : la page cesse d'être atteignable dans le délai de FR-111, mais ses images déjà produites restent stockées et résolvables pour qui en détient l'adresse directe. Le geste qui efface réellement le fichier est celui qui n'existe pas en v1. *(FR-083, FR-093, FR-111)*
...
- **Définition de formulaire modifiée entre l'affichage et l'envoi.** Le site public étant statique et potentiellement en cache, le visiteur peut envoyer une soumission calculée sur des prix qui viennent de changer. Le système **achemine** la demande avec le total recalculé (FR-091) sans rien signaler au visiteur — le total est indicatif par nature (FR-051) — et l'e-mail mentionne les prix utilisés. *(Le renoncement que cela emporte est écrit : le visiteur peut avoir vu un total quand l'éditrice en reçoit un autre, sans trace de part ni d'autre — FR-095 étant retirée, il ne lui reste aucun exemplaire. FR-051 est la parade et elle suffit, rien d'affiché n'étant opposable.)*
- **Soumission vers un formulaire retiré du site** *(2026-08-01)*. Le retrait (FR-083) ne supprime pas la définition publiée, et l'adresse d'envoi peut rester connue d'un ancien cache ou d'un signet. Le système **refuse** la soumission (FR-112) : une surface que l'éditrice croit fermée ne continue pas de collecter. Le formulaire remis en ligne redevient opérant sans que sa définition ait bougé.
...
- **SC-004** — Une publication est visible en ligne en moins de 5 minutes — et, **symétriquement, un retrait cesse d'y être visible dans le même délai** *(2026-08-01, FR-111)*. *Servi par* : FR-034, FR-036, FR-093, FR-111. **Mesuré en conditions nominales, sur un site à volume de contenu réaliste** (galeries remplies), et non sur un site fraîchement provisionné ; la première mise en ligne est exclue de la mesure.
...
- **SC-007** — La cliente construit un formulaire de devis elle-même, et une demande composée par un visiteur lui parvient par e-mail, avec réponses, total et coordonnées. *Servi par* : FR-040 à FR-046, FR-061, FR-086, FR-091, FR-094, FR-096 à FR-099, FR-101, FR-102, FR-105, FR-109, FR-113.
