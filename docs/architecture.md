...
> promotion dans `docs/adr/_candidates/`). La colonne **« Fondé par »** de la table le nomme, sous
> l'une ou l'autre forme. Ce document dit *ce que* le code doit tenir ; l'ADR dit *pourquoi*. Ne pas
> rejuger un invariant ici : l'arbitrage vit dans l'ADR.
>
> **Source.** `I1`–`I10` sont repris de [`docs/legacy/1.x/archi.md`](./legacy/1.x/archi.md)
> (invariants de structure), lui-même tracé vers le PRD et la Stack du cycle 1.x ; `I11`–`I13` sont
> nés en 2.x, d'ADR promus. À ne pas confondre avec les invariants **commerciaux** `I1`–`I6` du
> [socle de livraison](./legacy/socle-de-livraison.md) — aucun rapport de numérotation.
