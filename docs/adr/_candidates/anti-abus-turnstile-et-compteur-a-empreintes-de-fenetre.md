...
Oui, **à moitié**. La moitié statique — `idFromName` ne reçoit qu'une constante littérale — est rendue par `arch-invariants` sous `I7`. L'**effacement conjoint** de la clé de fenêtre et des entrées n'est rendu par aucun contrôle, et `docs/ci.md` le nomme comme tel.
