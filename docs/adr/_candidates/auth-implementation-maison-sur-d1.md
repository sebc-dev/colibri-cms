# Candidat ADR : Authentification — implémentation maison sur D1, quatre mécanismes
Statut : Candidat | Date : 2026-08-23 | Provenance : `docs/1.x/adr/0006-auth-implementation-maison-sur-d1.md` (ADR-0006 accepté sous le cycle 1.x)
...
Oui, **à moitié**. `arch-invariants` lit les quatre attributs du cookie de session — `__Host-`, `HttpOnly`, `Secure`, `SameSite=Strict` — sous `src/platform/session/` : ce sont des chaînes littérales. Le jeton anti-CSRF « sur **chaque** écriture » et le contrôle d'en-tête `Origin` sont une **couverture**, qui ne se grep pas — aucun contrôle ne les rend aujourd'hui.
