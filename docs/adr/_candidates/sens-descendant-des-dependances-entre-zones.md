...
Statut : Candidat | Date : 2026-08-23 | Provenance : `docs/1.x/adr/0021-sens-descendant-des-dependances-entre-zones.md` (ADR-0021 accepté sous le cycle 1.x)
...
[ADR-0001](../../1.x/adr/0001-cible-de-deploiement-worker-unique-workers-builds.md) a retenu **un artefact
...
routage imposée par Astro ([ADR-0002](../../1.x/adr/0002-generateur-astro-7.md)).
...
- **Le graphe d'imports du site publié n'atteint jamais la base.** Un build depuis les seuls
  ...
  [socle de livraison](../../socle-de-livraison.md) exige, et que `FR-107` et `SC-011` mesurent.
...
- **Tenir la frontière par le déploiement** (deux artefacts, l'un public l'autre
  d'administration) : écartée par [ADR-0001](../../1.x/adr/0001-cible-de-deploiement-worker-unique-workers-builds.md)
...
- **Ne poser aucune règle de sens et s'en remettre à la convention à la relecture** : écartée car
  le [Brief](../../1.x/brief.md) pose que « le code entrant n'est pas relu ligne à ligne ». Une
...
- **Des zones sans matrice, avec pour seul interdit l'isolement de `core/`** : écartée car moins
  ...
  ([ADR-0002](../../1.x/adr/0002-generateur-astro-7.md)), du code d'administration partirait dans le
