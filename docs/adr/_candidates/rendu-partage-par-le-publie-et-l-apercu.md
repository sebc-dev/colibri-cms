...
Oui, **partiellement** — `arch-invariants` rend `I3` pour ce qu'une expression régulière atteint ; le point d'entrée unique de `src/render/` se vérifie sur le graphe d'imports résolu, donc par `boundaries`.
